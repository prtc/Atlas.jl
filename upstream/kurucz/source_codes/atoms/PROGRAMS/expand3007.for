...
C    1' even',I7,' odd levels',I10,' ion pot cm-1')
  ...
  333 READ(8,2)EVEODD(K),EJE(K),XJE(K),ELABJE(K),GLANDEE(K),ASUME(K),
     1STARKE(K),WAALSE(K)
      DO 339 K=1,NO
  339 READ(8,7)ELABO(K),EXTRAJO(K)
      DO 338 K=1,NE
  338 READ(8,7)ELABE(K),EXTRAJE(K)
      ELSE
      DO 3 K=1,NE
...
C     PRINT 5,WL,GFLOG,KE,KO,IFPRED
    5 FORMAT(F18.8,F7.3,I5,I5,I1,I4)
      ...
      REF='K08 '
      REF='K09 '
      IF(EVEODD(KE).EQ.'EPo')REF='K09P'
      IF(EVEODD(KE).EQ.'ERy')REF='K09R'
      IF(ODDEVE(KO).EQ.'OPo')REF='K09P'
      IF(ODDEVE(KO).EQ.'ORy')REF='K09R'
C
...
C     WAVENO=ABS(ABS(EJE(KE))-ABS(EJO(KO)))
      ...
      GUP=XJUP+XJUP+1.
      GF=10**GFLOG
      A=GF*66702.D9/WLVAC**2/GUP
      BRANCHINGFRAC=0.
      IF(ASUM.GT.0.)BRANCHINGFRAC=A/ASUM
      FLOG=GFLOG-LOG10(GLO)
      FEMISSLOG=GFLOG-LOG10(GUP)
      GALOG=GFLOG+LOG10(66702.D9/WLVAC**2)
      ALOG=GALOG-LOG10(GUP)
C
      IF(IFPRED.EQ.0)THEN
C     WRITE(11,140)WL,GFLOG,CODE,EJE(KE),XJE(KE),ELABJE(KE),EJO(KO),
C    1 XJO(KO),ELABJO(KO),GAMMAR,GAMMAS,GAMMAW,REF,NBLO,NBUP,ISO1,X1,
      WRITE(11,140)WL,GFLOG,CODE,ELAB,ELABP,
     1 GAMMAR,GAMMAS,GAMMAW,REF,NBLO,NBUP,ISO1,X1,
     2 ISO2,X2,OTHER1,OTHER2,LANDE,LANDEP,ISOSHIFT
      ENDIF
C
      WRITE(12,140)WL,GFLOG,CODE,ELAB,ELABP
C
      IF(WAVEN.LE.9999999.999D0)THEN
      WRITE(15,144)WW,WAVEN,GFLOG,FLOG,FEMISSLOG,ALOG,GALOG,CODE,
     1 ELAB,ELABP
  144 FORMAT(F12.4,F12.3,5F7.3,F6.2,A28,A28)
      GO TO 7144
      ENDIF
      IF(WAVEN.LE.99999999.99D0)THEN
      WRITE(15,4144)WW,WAVEN,GFLOG,FLOG,FEMISSLOG,ALOG,GALOG,CODE,
     1 ELAB,ELABP
 4144 FORMAT(F12.4,F12.2,5F7.3,F6.2,A28,A28)
      GO TO 7144
      ENDIF
      IF(WAVEN.LE.999999999.9D0)THEN
      WRITE(15,5144)WW,WAVEN,GFLOG,FLOG,FEMISSLOG,ALOG,GALOG,CODE,
     1 ELAB,ELABP
 5144 FORMAT(F12.4,F12.1,5F7.3,F6.2,A28,A28)
      GO TO 7144
      ENDIF
      WRITE(15,6144)WW,WAVEN,GFLOG,FLOG,FEMISSLOG,ALOG,GALOG,CODE,
     1 ELAB,ELABP
 6144 FORMAT(F12.4,F12.0,5F7.3,F6.2,A28,A28)
 7144 CONTINUE
C
      WRITE(13,140)WL,GFLOG,CODE,ELAB,ELABP,
     1 GAMMAR,GAMMAS,GAMMAW,REF,NBLO,NBUP,ISO1,X1,
     2 ISO2,X2,OTHER1,OTHER2,LANDE,LANDEP,ISOSHIFT
C
      IF(ABS(EJE(KE)).GT.EION)GO TO 99
      IF(ABS(EJO(KO)).GT.EION)GO TO 99
      WRITE(14,140)WL,GFLOG,CODE,ELAB,ELABP,
     1 GAMMAR,GAMMAS,GAMMAW,REF,NBLO,NBUP,ISO1,X1,
     2 ISO2,X2,OTHER1,OTHER2,LANDE,LANDEP,ISOSHIFT
C 140 FORMAT(F11.4,F7.3,F6.2,F12.3,F5.1,1X,A10,F12.3,F5.1,1X,A10,
  140 FORMAT(F11.4,F7.3,F6.2,A28,A28,

...
C     TAPE12  GAM*E.META or *.WNMETA       subset of GA*.LINES with metastable levels
C     TAPE13  GFM*E.POS or *.WNPOS         subset of GA*.LINES with good wl, cutoff -10
C     TAPE14  GFM*E.METAPOS or *.WNMETAPOS subset of GA*.META with good wl
C
C     TAPE21  GAM*O.LINES or *.WNLINES     output lines
C     TAPE22  GAM*O.META or *.WNMETA       subset of GA*.LINES with metastable levels
C     TAPE23  GFM*O.POS or *.WNPOS         subset of GA*.LINES with good wl, cutoff -10
C     TAPE24  GFM*O.METAPOS or *.WNMETAPOS subset of GA*.META with good wl
C
C     TAPE31  GAQ*E.LINES or *.WNLINES     output lines
C     TAPE32  GAQ*E.META or *.WNMETA       subset of GA*.LINES with metastable levels
C     TAPE33  GFQ*E.POS or *.WNPOS         subset of GA*.LINES with good wl, cutoff -10
C     TAPE34  GFQ*E.METAPOS or *.WNMETAPOS subset of GA*.META with good wl
C
C     TAPE41  GAQ*O.LINES or *.WNLINES     output lines
C     TAPE42  GAQ*O.META or *.WNMETA       subset of GA*.LINES with metastable levels
C     TAPE43  GFQ*O.POS or *.WNPOS         subset of GA*.LINES with good wl, cutoff -10
...
C     .POS
      ...
      IF(GFLOG.LT.-9.999.AND.GAMMAR.GT.4.)GO TO 99
      WRITE(JT3,140)WL,GFLOG,CODE,ELAB,ELABP,GAMMAR,GAMMAS,GAMMAW,REF,
     1 NBLO,NBUP,ISO1,X1,ISO2,X2,OTHER1,OTHER2,LANDE,LANDEP,ISOSHIFT
C     .METAPOS

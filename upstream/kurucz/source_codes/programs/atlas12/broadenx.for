...
CBLUE      .25       .125      .1        .1        .05
    ...
    2 FORMAT(1X,A10,F10.2,A10,F10.2,A50)
      FWHM=-1.
      IF(B.EQ.'PM        ')FWHM=X1/WCEN/1000.*299792.458
      IF(B.EQ.'KM        ')FWHM=X1
      IF(B.EQ.'RESOLUTION')FWHM=299792.458/X1
      IF(B.EQ.'CM-1      ')FWHM=X1/(1.E7/WCEN)*299792.458
      IF(FWHM.LT.0.)THEN
      WRITE(6,3)
      CALL ABORT
      ENDIF
      ...
      FWHM2=FWHM
      IF(X2.GT.0.)THEN
      IF(B.EQ.'PM        ')FWHM2=X2/WCEN/1000.*299792.458
      IF(B.EQ.'KM        ')FWHM2=X2
      IF(B.EQ.'RESO      ')FWHM2=299792.458/X2
      IF(B.EQ.'CM-1      ')FWHM2=X2/(1.E7/WCEN)*299792.458
      ENDIF
...
C     GAUSSIAN INSTRUMENTAL PROFILE HALF WIDTH IN KM  FWHM
   ...
   24 BLUE(I)=RED(I)
      IF(X2.EQ.0.)GO TO 50
      DO 25 I=1,20000
      RED2(I)=EXP(-(FLOAT(I-1)*VSTEP/FWHM2*.8325546*2.)**2)
...
C     APODIZED BY EXP(-0.06*X**2)
   ...
   64 BLUE(I)=RED(I)
      ...
      RED2(1)=0.5
      DO 65 I=2,20000
      X=(FLOAT(I-1)*VSTEP/FWHM2*2.*1.8954942)
      RED2(I)=SIN(X)/X*EXP(-0.06*X**2)
      IF(ABS(RED2(I))+ABS(RED2(I-1)).LT.1.E-5)GO TO 66
   65 CONTINUE
   66 NPROF=I
      SUM=0.
      DO 67 I=1,NPROF

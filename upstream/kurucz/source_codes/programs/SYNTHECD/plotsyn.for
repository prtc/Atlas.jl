...
C     CALL RDYOUTF(6,0)
 ...
 1001 FORMAT(10I8)
     ...
     1NOWAVE,NOCALC,NOLABY
 1002 FORMAT(1X,10I8/'   IFLABL  IFABSO  IFCONT  IFGRID  IFDLIN',
     1'   IFLOG   JUST1  NOWAVE  NOCALC  NOLABY')
      READ(5,1001)
 ...
 1003 FORMAT(1X,10I8/'   IFNOAX    IFMU  NOPRNT  IFKPNO   IFKPK',
     1'  IFSACP  IFHAWA   IFNRL  IFPROC IFSIRUV')
      READ(5,1001)
 ...
 1027 FORMAT(8F10.3)
     ...
     1AXISWT
 1028 FORMAT(8F10.3/79H DOPTERR   SCALOBS   ZEROOBS   RMIN2     RMAX2
     1  XOFFSET   SCALOB2   AXISWT     )
      READ(5,1001)
      READ(5,1009)IFPANL
 1009 FORMAT(80I1)
      WRITE(6,1010)IFPANL
...
C      CALL STRINGX10(WW6,20.5,14.35-OFFSET+CANON)
      CALL STRINGY10(WW6,-.2,12.00-OFFSET+CANON)
      CALL STRINGY10(WW6,23.6,12.00-OFFSET+CANON)
...
C      CALL STRINGX(HEADERDATA(5),3.5,14.55-OFFSET+CANON)
      CALL STRINGY(HEADERDATA(1),-.25,11.00-OFFSET+CANON)
      CALL STRINGY(HEADERDATA(3),-.40,11.00-OFFSET+CANON)
      CALL STRINGY(HEADERDATA(4),-.55,11.00-OFFSET+CANON)
      CALL STRINGY(HEADERDATA(5),-.70,11.00-OFFSET+CANON)
      ENDIF
...
C
   ...
   25 FORMAT(1P2E12.3)
C      IF(NOLABY.EQ.1)GO TO 270
C      ENCODE(9,3535,STRING9)HMAX
C      CALL STRINGY2(STRING9,END+.6,YTOP-1.2)
C      ENCODE(9,3535,STRING9)HMIN
...
CCRAY IF(NIN.GT.100000)CALL ABORT
    ...
    9 IF(NIN.EQ.0)RETURN
      CALL WEIGHT(2)
      IF(IFSUNF.EQ.2.OR.IFSUNF.EQ.12)CALL WEIGHT(MINWT)
      ISTART=0
      DO 14 I=1,NIN
      W=WI(I)
      IF(W.LT.W1)GO TO 14
...
C      NCELL=(WAVEL-W1)*10.*CELLIN*XSCALE+1.5
   ...
   63 NCELL=I
      ...
      YCELL=YTOP+.1
      IF(IFLABL.EQ.1.AND.MOD(NCELL,2).EQ.0)THEN
      YCELL=YTOP+2.81
      IF(IFCELL(NCELL-1,MROW).EQ.0)THEN
      IFCELL(NCELL-1,MROW)=1
      IFCELL(NCELL,MROW)=0
      YCELL=YTOP+.1
      XCELL=FLOAT(NCELL-1)/CELLIN
      ENDIF      
      ENDIF
C      IF(IFLABL.EQ.1.AND.MOD(NCELL,2).EQ.0)YCELL=YTOP+2.00
C     IF(IFLABL.EQ.1.AND.MOD(NCELL,2).EQ.0)YCELL=YTOP+1.80
C      IF(IFLABL.EQ.31.AND.CODE.GT.100.)YCELL=YTOP+1.80
C      IF(IFLABL.EQ.31.AND.CODE.GT.100.)YCELL=YTOP+2.40
      IF(IFLABL.EQ.31.AND.CODE.GT.100.)YCELL=YTOP+2.81
...
C
      ...
      MOL=MOLAF(ISOAF)
      NAME=NAMEAF(ISOAF)
      WRITE(STRING21,'(I3.3,1X,A6,I5,I4,2X)')IWL,NAME,IELO,IRESID
C      IF(IFLABL.NE.21)GO TO 66
...
C 6700 L34=LABAF(25:28)//'-'//LABAF(35:36)//LABAF(10:16)
 6800 CONTINUE
      IF(CODE.EQ.10108.)L34=L34(1:6)//L34(8:13)//L34(15:19)
      IF(CODE.EQ.808.)
     1 L34=L34(1:5)//'-'//L34(8:10)//L34(16:17)//L34(12:14)
...
C     IF(IFLABL.NE.31.AND.STRING9.EQ.'  +0   +0')STRING9='         '
  ...
  113 FORMAT(I3.3,A1,I3,1X,A2,I3,1H-,I2,I3,A1,I4,A9)
      CALL STRINGY(STRING33,XCELL,YCELL)
      GO TO 67
   64 CONTINUE
C      IF(IFLABL.EQ.31)THEN
      IDWL=NINT(DWL*10000.)
      IDGFLOG=NINT(DGFLOG*1000.)
      WRITE(STRING9,'(SPI4,SPI5)')IDWL,IDGFLOG
C     IF(IFLABL.NE.31.AND.STRING9.EQ.'  +0   +0')STRING9='         '
      IF(STRING9.EQ.'  +0   +0')STRING9='         '
      WRITE(STRING30,7114)IWL,COLON,CODE,IELO,IRESID,STRING9
 7114 FORMAT(I3.3,A1,F6.2,I7,I4,A9)
      CALL STRINGY(STRING30,XCELL,YCELL)
C      GO TO 67
C      ENDIF
C      WRITE(STRING21,114)IWL,COLON,CODE,IELO,IRESID
C  114 FORMAT(I3.3,A1,F6.2,I7,I4)
C   66 CONTINUE
C      CALL STRINGY(STRING21,XCELL,YCELL)

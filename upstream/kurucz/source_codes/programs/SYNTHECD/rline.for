      PROGRAM RLINE
c     revised 13feb93
C     PROGRAM RLINE(TAPE11,OUTPUT,TAPE6=OUTPUT,TAPE12,TAPE13,TAPE93)
...
C     DWL A CORRRECTION TO THE WAVELENGTH
C     DGFLOG A CORRECTION TO THE LOGGF
C     DGAMMAR A LOG CORRECTION TO GAMMAR
C     DGAMMAS A LOG CORRECTION TO GAMMAS
C     DGAMMAW A LOG CORRECTION TO GAMMAW
C     DWLISO   ISOTOPE SHIFT
C     SAMPLE CARDS
...
C      WRITE(6,7777)WLBEG,WLEND,RESOLU,RATIO,RATIOLG,CUTOFF
C 7777 FORMAT(1P6E15.8)
      ...
      DELFAC=1.
      IF(WLBEG.GE.1000.)DELFAC=2.
      IF(WLBEG.GE.3000.)DELFAC=4.
  ...
  140 FORMAT(F10.4,F7.3,F5.1,F12.3,F5.1,F12.3,F9.2,A8,A2,A8,A2,F7.3/
     1F10.4,I4,F6.2,F6.2,F6.2,A4,I2,I2,I3,F7.3,I3,F7.3,A8,A2,A8,A2,
     2F7.4,F7.3,3F6.2,A20)
      WLVAC=DABS(WL)+DWL+DWLISO
      IF(IFVAC.EQ.1)WLVAC=1.E7/DABS(DABS(EP)-DABS(E))+DWL+DWLISO
      DECODE(10,143,OTHER2)O1,NKP
  143 FORMAT(A1,3X,I6)
      LIM=7
C     IF(O1.EQ.1H )LIM=7
C     IF(O1.EQ.1H0)LIM=7
C     IF(O1.EQ.1H1)LIM=7
      IF(O1.EQ.1H2)LIM=6
      IF(O1.EQ.1H3)LIM=5
      IF(O1.EQ.1H4)LIM=4
      IF(O1.EQ.1H5)LIM=3
      IF(O1.EQ.1H6)LIM=2
      IF(O1.EQ.1H7)LIM=1
      IF(WLVAC.LT.WLBEG-DELLIM(LIM)*DELFAC)GO TO 900
      IF(WLVAC.GT.WLEND+DELLIM(LIM)*DELFAC)GO TO 900
...
c     IF((CODE.LT.100.).AND.(CODE.NE.26.01).AND.(NKP.GT.0).AND.
      IF((CODE.LT.100.).AND.(NKP.GT.0).AND.
     1(WL.GT.0.).AND.
     2(MOD(NDELET(NKP/10+1)/NTENS(MOD(NKP,10)+1),10).EQ.0))
     3NDELET(NKP/10+1)=NDELET(NKP/10+1)+NTENS(MOD(NKP,10)+1)
